...
2. 双击项目下载目录中的 `START-EXPORT-TO-USB.cmd`（macOS 源端使用 `START-EXPORT-TO-USB.command`）。
3. 程序自动发现 Codex 注册的实际项目路径，并同时收集 `.codex`、每个项目和可移植桌面状态。未注册但需要迁移的目录可用高级 `--project NAME=PATH` 方式补充。
4. 完成时 U 盘上会出现 `Codex-Migration-Package-时间戳`。保留整个文件夹，不能只复制其中某个 tar 文件。
